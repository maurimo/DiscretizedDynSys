...
class PerturbedFourxDynamic(Mod1Dynamic):
	r"""
	Dynamic :math:`4x+c\sin(j\pi x)+k \mod 1`, for tunable parameters ``c`` and ``j``.
	"""
	def __init__(self, prec=53, c='0.01', j=2, k='0'):
		"""
		Args:
			
		"""
		nbranches = 4 if k == '0' else 5
		Mod1Dynamic.__init__(self, nbranches, prec)
	...
	def f_unquotiented(self, x):
		...
		warn_if_not_in_01(x)
		y=self._four * x + self._c * (self._jpi*x).sin() + self._k
...
class PerturbedQxDynamic(Mod1Dynamic):
	r"""
	Dynamic :math:`Qx+c\sin(j\pi x)+k \mod 1`, for tunable parameters ``Q``, ``k``, ``c`` and ``j``.
	"""
	def __init__(self, prec=53, Q='3', c='0.01', j=2, k='0'):
		"""
		Args:
			
		"""
		nbranches = int(Q) if k == '0' else int(Q)+1
		Mod1Dynamic.__init__(self, nbranches, prec)
		self._Q = self.field(Q)
		self._c = self.field(c)
		self._k = self.field(k)
		self._jpi = self.field.pi() * j
		self._j=j
		infTprime = self._Q - self._c*self._jpi
		if not infTprime > 1:
			warn("The dynamic is not expanding with this choice of the parameters")
	...
	def f_unquotiented(self, x):
		...
		warn_if_not_interval(x)
		warn_if_not_in_01(x)
		y=self._Q * x + self._c * (self._jpi*x).sin() + self._k
		#if (self._j%4==0): #since points of the  type i/4 are sent into integers, we can use this information to get better bounds on the computation  
		#	if ((x.upper()*self._j).is_integer()):
		#		a=y.unique_integer()
		#		y=y.intersection(self._intervalfield(a-1,a))
	...
	def fprime(self, x):
		warn_if_not_interval(x)
		warn_if_not_in_01(x)
		return self._Q + self._c * self._jpi * (self._jpi * x).cos()

	def expansivity(self):
		return self._Q - self._c*self._jpi
...
class PerturbedQWxDynamic(Mod1Dynamic):
	r"""
	Dynamic :math:`Qx+c\sin(j\pi x)+c2sin(j2\pi x)+k \mod 1`, for tunable parameters ``Q``, ``k``, ``c`` and ``j``.
	"""
	def __init__(self, prec=53, Q='3', c='0.01', j=2, c2='0.01', j2=2, k='0'):
		"""
		Args:
		...
		self._jpi = self.field.pi() * j
		self._j=j
		self._c2 = self.field(c2)
		self._j2pi = self.field.pi() * j2
		self._j2=j2
		self._f_Q = float(self._Q.center())
		self._f_k = float(self._k.center())
		self._f_c = float(self._c.center())
		self._f_jpi = float(self._jpi.center())
		self._f_j = self._j
		self._f_c2 = float(self._c2.center())
		self._f_j2pi = float(self._j2pi.center())
		self._f_j2 = self._j2
		infTprime = self._Q - self._c*self._jpi - self._c2*self._j2pi
		if not infTprime > 1:
			warn("The dynamic is not expanding with this choice of the parameters")
	...
	def f_unquotiented(self, x):
		...
		warn_if_not_interval(x)
		warn_if_not_in_01(x)
		y=self._Q * x + self._c * (self._jpi*x).sin() + self._c2 * (self._j2pi*x).sin() + self._k
		return y
		
	def fprime(self, x):
		warn_if_not_interval(x)
		warn_if_not_in_01(x)
		return self._Q + self._c * self._jpi * (self._jpi * x).cos() + self._c2 * self._j2pi * (self._j2pi * x).cos()

	def expansivity(self):
		return self._Q - self._c*self._jpi - self._c2*self._j2pi

	def distorsion(self):
		return (self._c * (self._jpi**2) + self._c2 * (self._j2pi**2)) / self.expansivity()**2

	def third_distorsion(self):
		return (self._c * self._jpi * self._jpi* self._jpi + self._c2 * self._j2pi * self._j2pi* self._j2pi) / self.expansivity()**3
	...
	def fsecond(self, x):
		warn_if_not_interval(x)
		return - self._c * (self._jpi**2) * (self._jpi * x).sin() - self._c2 * (self._j2pi**2) * (self._j2pi * x).sin()
	
	def fthird(self, x):
		warn_if_not_interval(x)
		return - self._c * (self._jpi**3) * (self._jpi * x).cos() - self._c2 * (self._j2pi**3) * (self._j2pi * x).cos()
	
	def quick_f(self, x):
		return math.fmod(self._f_Q * x + self._f_c * math.sin(self._f_jpi*x) + \
					self._f_c2 * math.sin(self._f_j2pi*x) + self._f_k, 1.0)
		
	def quick_fprime(self, x):
		return self._f_Q + self._f_c * self._f_jpi * math.cos(self._f_jpi * x) + \
				self._f_c2 * self._f_j2pi * math.cos(self._f_j2pi * x)


class PerturbedTwoxDynamic(Mod1Dynamic):
	r"""
	Dynamic :math:`2x+c\sin(j\pi x)+k \mod 1`, for tunable parameters ``c`` and ``j``.
	"""
	def __init__(self, prec=53, c='0.01', j=2, k='0'):
		"""
		Args:
		...
		"""
		nbranches = 2 if k == '0' else 4
		Mod1Dynamic.__init__(self, nbranches, prec)
		self._two = self.field('2')
		self._c = self.field(c)
		self._k = self.field(k)
		self._jpi = self.field.pi() * j
		self._j=j
		infTprime = self._two - self._c*self._jpi
		if not infTprime > 1:
			warn("The dynamic is not expanding with this choice of the parameters")
	...
	def f_unquotiented(self, x):
		...
		warn_if_not_interval(x)
		warn_if_not_in_01(x)
		y=self._two * x + self._c * (self._jpi*x).sin() + self._k
		#if (self._j%4==0): #since points of the  type i/4 are sent into integers, we can use this information to get better bounds on the computation  
		#	if ((x.upper()*self._j).is_integer()):
		#		a=y.unique_integer()
		#		y=y.intersection(self._intervalfield(a-1,a))
	...
	def fprime(self, x):
		warn_if_not_interval(x)
		warn_if_not_in_01(x)
		return self._two + self._c * self._jpi * (self._jpi * x).cos()

	def expansivity(self):
		return self._two - self._c*self._jpi
